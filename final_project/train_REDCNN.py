...
from data_utils import create_img_pairs, prepare_data, split_img_pairs, ImgPairsDataset
...
if __name__ == "__main__":
    ...
    # define arguments
    parser.add_argument('--experiments_path', type=str, default='/home/DATA_Lia/data_06/CT_Enhancement/')
    parser.add_argument('--csv_path', type=str, default='./all_imgs.csv')
    parser.add_argument('--data_path', type=str, default='/home/DATA_Lia/data_06/Data_Kosin_JPG_Modified/')
    ...
    parser.add_argument('--model', type=str, default='REDCNN-L1')
    ...
    save_path = os.path.join(args.experiments_path, args.model)
    ...
    for epoch in range(args.num_epochs):
        ...
        # training
        for i, (ldct_img, sdct_img, descr, img_name) in enumerate(tqdm(train_dataloader)):
            ...
            optimizer.zero_grad()
            # forward pass
            output = model(ldct_img)
            # compute losses
            loss = loss_fn(output, sdct_img)
            # backpropagation
            loss.backward()
            # update weights
            optimizer.step()
        ...
        training_losses[epoch] = {'loss': running_loss / len(train_dataloader)}
        ...
        with torch.no_grad():
            ...
            for i, (ldct_img, sdct_img, descr, img_name) in enumerate(tqdm(val_dataloader)):
                ldct_img = ldct_img.to(device)
                sdct_img = sdct_img.to(device)
            ...
            print('{} output vs SDCT:\n\tMSE: {:.4f}, \n\tRMSE: {:.4f} \n\tPSNR: {:.4f} \n\tSSIM: {:.4f}'
                  .format(args.model,
                          output_sdct_mse / len(val_dataloader), 
